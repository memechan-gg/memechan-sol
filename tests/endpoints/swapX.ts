...
export function test() {
  describe("swap_x", () => {
    ...
    beforeEach("swap Y", async () => {
      ticket = await pool.swapY({
        owner: user,
        coinXMinValue: new BN(1),
        coinInAmount: new BN(10_000 * 1e9), // target is 40_000
      });
    });

    it("swaps user quote->memecoin->quote", async () => {
      const ticketInto = await ticket.fetch();

      await sleep(2000);

      await pool.swapX({
        owner: user,
        memeTicket: ticket.id,
        coinInAmount: ticketInto.amount,
        coinYMinValue: new BN(1),
      });
    });
