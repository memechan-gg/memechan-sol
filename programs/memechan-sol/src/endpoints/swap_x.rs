...
pub fn handle(ctx: Context<SwapCoinX>, coin_in_amount: u64, coin_y_min_value: u64) -> Result<()> {
    ...
    pool_state.protocol_fees_meme += swap_amount.protocol_fee_in;
    pool_state.protocol_fees_quote += swap_amount.protocol_fee_out;

    pool_state.meme_reserve.tokens += swap_amount.amount_in;
    pool_state.quote_reserve.tokens -= swap_amount.amount_out + swap_amount.protocol_fee_out;
